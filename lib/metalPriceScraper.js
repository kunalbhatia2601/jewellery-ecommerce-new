...
const BULLIONS_URL = 'https://bullions.co.in/location/bhopal/';
 ...
 * SECURITY: Only uses server-side GEMINI_API_KEY (never NEXT_PUBLIC_ variant)
 */
function initGeminiAI() {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) {
    console.warn('⚠️  GEMINI_API_KEY not found - using basic extraction fallback');
    console.warn('⚠️  Set GEMINI_API_KEY in your .env file (NOT NEXT_PUBLIC_GEMINI_API_KEY)');
    return null;
  }
  try {
...
async function scrapeBullionsPrices() {
  try {
    ...
    // Parse with JSDOM - disable scripts to avoid errors from external scripts
    const dom = new JSDOM(response.data, {
      url: BULLIONS_URL,
      runScripts: 'outside-only', // Don't execute scripts to avoid ad/tracking errors
      resources: 'usable',
      pretendToBeVisual: true,
      virtualConsole: new (require('jsdom').VirtualConsole)() // Suppress console errors
    });
...
export async function fetchLiveMetalPrices(currency = 'INR') {
  try {
    ...
    const base999Silver = cleanedData.silver['999'] || 0;
    
    // Calculate silver prices for different purities based on base 999 price
    const silverPrices = {
    ...
    const structuredData = {
      ...
      rates: {
        ...
        silver: base999Silver // Base 999 silver price
      },
      perGram: {
        gold: cleanedData.gold['24k'] || cleanedData.gold['22k'] || 0,
        silver: base999Silver // Base 999 silver price
      ...
      detailed: {
        ...
        silver: {
          '999': silverPrices['999'] || null,
          '925': silverPrices['925'] || null,
          '700': silverPrices['700'] || null,
          '650': silverPrices['650'] || null
        }
      }
    };
...
function getFallbackMetalPrices(currency = 'INR') {
  ...
  const base999Silver = 95; // INR per gram (999 purity)
  ...
  return {
    ...
    rates: {
      ...
      silver: base999Silver  // INR per gram (999)
    },
    perGram: {
      gold: 7200,
...
// Default export
const metalPriceScraper = {
  fetchLiveMetalPrices,
  getCachedMetalPrice,
  clearPriceCache
};

export default metalPriceScraper;
