...
export default function ProductForm({ product, onSubmit, onCancel }) {
    ...
    const calculateDynamicPrice = async () => {
        ...
        try {
            ...
            // Get stone value - either from total stones or manual input
            const totalStoneValue = parseFloat(formData.stoneValue) || 0;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Validate variant stock if variants are enabled
            if (formData.hasVariants && formData.variants.length > 0) {
                const totalStock = parseInt(formData.stock) || 0;
                const variantStockSum = formData.variants.reduce((sum, variant) => {
    ...
    return (
        <div className="max-w-7xl mx-auto">
            <form onSubmit={handleSubmit} className="space-y-8">
                ...
                {/* Section 2: Stone Specifications */}
                <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                    <div className="bg-gradient-to-r from-amber-600 to-yellow-600 px-6 py-4">
                        <div className="flex items-center justify-between">
                            ...
                            <button
                                type="button"
                                onClick={addStone}
                                className="px-4 py-2 bg-white text-amber-700 rounded-lg hover:bg-gray-100 transition-all flex items-center gap-2 font-medium shadow-sm"
                            >
                        ...
                        <p className="text-amber-100 text-sm mt-1">
                            Add diamonds, rubies, emeralds, and other precious gems to calculate total stone value
                        </p>
                    </div>
                    <div className="p-6">
                        {formData.stones.length === 0 ? (
                            <div className="bg-gradient-to-br from-gray-50 to-gray-100 border-2 border-dashed border-gray-300 rounded-xl p-12 text-center">
                                ...
                                <button
                                    type="button"
                                    onClick={addStone}
                                    className="inline-flex items-center gap-2 px-5 py-2.5 bg-amber-600 text-white rounded-lg hover:bg-amber-700 transition-all font-medium shadow-sm"
                                >
                                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                                    </svg>
                                ...
                                {calculatedPrice?.breakdown && (
                                    <div className="bg-white border-2 border-gray-200 rounded-lg p-5 space-y-3">
                                        ...
                                        <div className="space-y-2 text-sm">
                                            ...
                                            {calculatedPrice.breakdown.stoneValue > 0 && (
                                                <div className="flex justify-between py-1">
                                                    <span className="text-gray-600">Stone Value:</span>
                                                    <span className="font-medium">₹{Number(calculatedPrice.breakdown.stoneValue || 0).toFixed(2)}</span>
                                                </div>
                                            )}
                                            <div className="flex justify-between py-1">
